...
if __name__ == '__main__':
    ...
    metrics_dict['hitrate_at_5'] = metrics.hitrate_score(dataset['ground_truth'], dataset['prediction'])
    metrics_dict['map_at_3'] = metrics.mean_average_precision_score(dataset['ground_truth'], dataset['prediction'])
    metrics_dict['map_at_5'] = metrics.mean_average_precision_score(dataset['ground_truth'], dataset['prediction'])
    metrics_dict['ndcg_at_3'] = metrics.ndcg_score(dataset['ground_truth'], dataset['prediction'])
    metrics_dict['ndcg_at_5'] = metrics.ndcg_score(dataset['ground_truth'], dataset['prediction'])

    print("-- Metrics comparison --")
    print(f"{'Expected:':>28}", ""*10, "Estimated:")
    for mtrc_name, expected_value in TASK_0_EXPECTED_SCORES.items():
        estimated_value = metrics_dict.get(mtrc_name)
        if estimated_value is not None:
            print(f"{mtrc_name.capitalize()}: {expected_value:>{25-len(mtrc_name)}} {estimated_value:>6}")